 ...
 * 
 */
public class WXHttpClient implements java.io.Serializable {
	...
	static Logger log = Logger.getLogger(WXHttpClient.class.getName());
	...
	public WXHttpClient(int maxConPerHost, int conTimeOutMs, int soTimeOutMs,
			int maxSize) {
	...
	public Response get(String url, PostParameter[] params)
		...
		if (null != params && params.length > 0) {
			String encodedParams = WXHttpClient.encodeParameters(params);
			if (-1 == url.indexOf("?")) {
				url += "?" + encodedParams;
			} else {
				url += "&" + encodedParams;
		...
		if (null != paging) {
			...
			if (null != params) {
				...
				if (0 != arrayPagingParams.length) {
					String encodedParams = WXHttpClient
	...
	public Response httpRequest(HttpMethod method, Boolean WithTokenHeader)
		...
		try {
			...
			if (WithTokenHeader) {
				...
				headers.add(new Header("User-Agent", "Mozilla/5.0 (Linux; U; Android 2.3; en-us; DROID RAZR 4G Build/6.5.1-73_DHD-11_M1-29) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1"));
			...
			//String s=new String(response.getParameter().getBytes("ISO-8859-1;charset=utf-8"));
