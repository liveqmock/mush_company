...
}),wx.jslog=function(o,i,r){
r=r||3,o=jQuery.extend(o,t);
var s=[];
i&&jQuery.each(["message","stack","lineNumber"],function(e,t){
s.push(t+":"+(i[t]||""));
}),o.e=s.join(";").replace(/\s/g," ");
var a="[fakeid={fakeid}] [script] [url={url}] [useragent={userAgent}] [src={src}] [exception={e}]".format(o);
e(r,a),console&&console.error&&i&&console.error(i);
};
}(),setTimeout(function(){
...
var o="__draft__"+wx.data.uin,i="__draft__time__"+wx.data.uin;
t()&&(e.remove(o),e.remove(i));
});
});
...
for(var r in e){
var s=e[r];
"string"==typeof s?i(r)||(e[r]=o.sprintf(r)+s):"object"==typeof s&&wx.injectXss(e[r]);
}
}
},void jQuery(function(){
...
var t=document.getElementById("ZoomFlash").height,o=t,i=1;
try{
o=document.getElementById("ZoomFlash").getFlashStageRect().height,i=~window.navigator.userAgent.toLowerCase().indexOf("msie")&&screen.deviceXDPI&&screen.logicalXDPI?screen.deviceXDPI/screen.logicalXDPI:o/t,
.9>i?(jQuery("#zoom_msg").text("您的浏览器目前处于缩小状态，会导致公众平台网页显示不正常，您可以键盘按“ctrl+数字0”组合键恢复初始状态。"),
jQuery("#zoom_tips").show(),jQuery("body").addClass("scaled")):i>1.1?(jQuery("#zoom_msg").text("您的浏览器目前处于放大状态，会导致公众平台网页显示不正常，您可以键盘按“ctrl+数字0”组合键恢复初始状态。"),
...
}(window);;define("common/wx/Tips.js",[],function(e,s){
"use strict";
function n(e,s,n){
$(".JS_TIPS").remove(),t=$(template.compile('<div class="JS_TIPS page_tips {type}" id="wxTips_'+(new Date).getTime()+'"><div class="inner">{=msg}</div></div>')({
type:e||"error",
msg:s
})).appendTo("body").fadeIn(),i.delay(n||r.delay,null,t);
}
function i(e){
...
c.err=function(e,s){
n("error",e||r.errMsg,s);
},c.suc=function(e,s){
n("success",e||r.sucMsg,s);
};
...
e.dom.fadeIn(),t.draggable&&e.dom.draggable(),function(){
...
});;define("common/wx/popup.js",["common/wx/widgetBridge.js","tpl/popup.html.js"],function(t){
"use strict";
t("common/wx/widgetBridge.js");
...
var s;
if(this.element.is("script[type=text/html]"))s=this.element.html(),this.options.data&&this.options.template&&(s=this.options.template(s)(this.options.data));else{
var l=$("<div></div>").append(this.element.clone()),s=l.html();
this.options.data&&this.options.template&&(s=this.options.template(s)(this.options.data));
}
if(i.content=s,this.$dialogWrp=$(o(i)).appendTo("body"),this.$dialogWrp.find(".dialog_bd").children(":first").show(),
